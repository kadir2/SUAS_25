...
class CoordinateFinder():
    def __init__(self, img_shape: tuple, live_location: list, drone_yaw, focal_length, sensor_size, drone_altitude) -> None:
        ...
        try:    
            ...
            logger.debug("CoordinateFinder __init__ fonksiyonu başarıyla çalıştı.")
        except Exception as e:
            logger.debug("CoordinateFinder __init__ fonksiyonunda hata: {}".format(e))
    ...
    def find_hfov_vfov(self):
        try:
            ...
            logger.debug("find_hfov_vfov fonksiyonu başarıyla çalıştı.")
        except Exception as e:
            logger.debug("find_hfov_vfov fonksiyonunda hata: {}".format(e)) 
        return h_fov, v_fov
    ...
    def pixel_to_meter(self):
        try:
            ...
            logger.debug("pixel_to_meter fonksiyonu başarıyla çalıştı.")
        except Exception as e:
            logger.debug("pixel_to_meter fonksiyonunda hata: {}".format(e))
    ...
    def location_from_a_point(self, coord, dist, direction):
        if (coord is None) or (dist is None) or (direction is None):
            logger.debug("location_from_a_point fonksiyonu için argüman eksik.")
        try:
            destination = gp.distance(meters=dist).destination((coord[0], coord[1]), bearing=direction)
            logger.debug("location_from_a_point fonksiyonu başarıyla çalıştı.")
        except Exception as e:
            logger.debug("location_from_a_point fonksiyonunda hata: {}".format(e))
        return destination

    def radian_to_angle(self, angle):
        if angle is None:
            logger.debug("radian_to_angle fonksiyonu için argüman eksik.")
        logger.debug("radian_to_angle fonksiyonu çalıştı.")
    ...
    def calculate_roi_coord_with_line_len(self, distance_to_center, roi_angle, center):
        if (distance_to_center is None) or (roi_angle is None) or (center is None):
            logger.debug("calculate_roi_coord_with_line_len fonksiyonu için argüman eksik.")
        try:
            norm_angle = 360 - self.drone_yaw
            final_angle = roi_angle - norm_angle
            logger.debug("calculate_roi_coord_with_line_len fonksiyonu çalıştı.")
        except Exception as e:
            logger.debug("calculate_roi_coord_with_line_len fonksiyonunda hata: {}".format(e))
    ...
    def roi_geo_coord_finder(self, roi_pixel_coords):
        ...
        try:
            ...
            logger.debug("roi_geo_coord_finder fonksiyonu başarıyla çalıştı.")
        except Exception as e:
            logger.debug("roi_geo_coord_finder fonksiyonunda hata: {}".format(e))
        return roi_coords
...
def run_production_mode():
    ...
    try:
        # Check the 'sicti' value and break if it's True
        sicti = r.get('sicti')
        if sicti == b'True':  # Redis stores values as bytes, so check if it's 'True'
            logger.debug("feci sictik coord finder gümletti random drop yap")
            r.set('cf_done', 'True')  # Set the 'cf_done' signal
            sys.exit("Exiting due to sicti being True.")  # Exit the program if sicti is True
    except Exception as e:
    ...
    try: 
        ...
        logger.debug("Dosyalardan okuma sırasında hata: {}".format(e))
        return
    ...
    image_shape = (4032, 3040)   # (genişlik, yükseklik)
