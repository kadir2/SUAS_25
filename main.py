...
class System:
    ...
    def set_yaw(self,heading, relative=False):
        current_yaw_rad_initial = self.vehicle.attitude.yaw
        current_yaw_deg_initial = math.degrees(current_yaw_rad_initial)
        # 0-360 derece aralığına normalize et
        current_yaw_deg_initial = (current_yaw_deg_initial + 360) % 360
        if relative:
            ...
            diff_cw = (heading - current_yaw_deg_initial + 360) % 360
            # Saat yönünün tersine fark
            diff_ccw = (current_yaw_deg_initial - heading + 360) % 360

            if diff_cw <= diff_ccw:  # Saat yönü daha kısa veya eşitse (eşitlik durumu dahil)
                param3_value = 1  # Saat yönü (CW)

            else:  # Saat yönünün tersi daha kısaysa
                param3_value = -1 # Saat yönünün tersi (CCW)
    ...
    def get_vehicle_position(self):
        ...
        jsonheading = self.vehicle.heading  # heading drone'un yönü (yaw)
        # jsonbattery = f"{self.vehicle.battery}"
        # Dönüştürme: coordinate finder'ın beklediği formatta (image, lat, lon, alt, yaw)
    ...
    def sim_fake_drop(self,coords):
        ...
        for roi in coords:
            ...
            lat, lon = roi
            self.mavlink_handler.simple_go_to(lat,lon,30, block=True)
            time.sleep(2) #drop yapma süresi dümenden
            self.logger.debug(f"droplandı")
            count += 1
            print(f"count={count}")
            if count < 4:
                BottleDropHandler(vehicle=self.vehicle,handler=self.mavlink_handler, config=self.config).turn_lap()
            else:
                pass
