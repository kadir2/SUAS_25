...
def gstreamer_pipeline(
    ...
    """
    return (
        f"nvarguscamerasrc sensor-id={sensor_id} aelock=true awblock=true wbmode=7 exposurecompensation=-2 ee-mode=1 ee-strength=1 ! "
...
class Loggerr:
    def __init__(self):
        # Logger oluştur
        self.logger = logging.Logger('dronelogger')
        ...
        log_file_path = 'dronelogger.log'  # Log dosyasının adı
        old_logs_dir = "old_logs_dronelogger"  # Eski log dosyalarının taşınacağı klasör
        ...
        if os.path.exists(log_file_path):  # Eğer log dosyası varsa
            ...
            new_log_file_name = f"dronelogger_{timestamp}.log"  # Yeni log dosyasının adı
...
#     def __init__(self, vehicle, handler:MAVLinkHandler, config, drop_altitude=20, drop_delay=10):
...
#                 self.handler.simple_go_to(lat, lon, 20, block=True, distance_radius=1.5)
#                 logger.debug(f"Reached bottle drop: {i+1}")
...
#                 logger.debug(f"Object {i+1} dropped successfully")

#                 #print(f"Turn Lap: {i+2} Begin")                        # lap test aşamasında olmayacak
#                 #self.turn_lap()
            
#             logger.debug("All objects dropped successfully")
#         except Exception as e:
#             print(e)
#             logger.debug("Error occurred in drop_bottle function.")
...
#         while True:
#             time.sleep(1)
#             print(f"next wp: {self.vehicle.commands.next}")
#             print(f"wp_num:{wp_num}")
...
# --- Yeni klasör ve d
        ...
        logger.debug(f"Drop info: {self.drop_info}")
    ...
    def main(self):
        ...
        os.system("bash system_mappingsiz.sh") # Starts the processing system
        self.mavlink_handler.set_parameter_value('WP_YAW_BEHAVIOR', 1)
