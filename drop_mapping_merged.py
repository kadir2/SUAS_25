...
class Loggerr:
    def __init__(self):
        # Logger oluştur
        self.logger = logging.Logger('SUAS')
        ...
        log_file_path = 'SUAS.log'  # Log dosyasının adı
        ...
        if os.path.exists(log_file_path):  # Eğer log dosyası varsa
            ...
            new_log_file_name = f"SUAS_{timestamp}.log"  # Yeni log dosyasının adı
...
# --- Yeni klasör ve d
    ...
    def fake_drop(self,coords):
        ...
        for roi in coords:
            ...
            lat, lon = roi
            self.mavlink_handler.simple_go_to(lat,lon,20, block=True, distance_radius=1.5)
    ...
    def real_drop(self, coords):
        ...
        for idx, (lat, lon) in enumerate(coords):            
            ...
            # Hedefe git
            self.mavlink_handler.simple_go_to(lat, lon, 17, block=True, distance_radius=1.5)
            # Servo bilgisini al
            try:
    ...
    def main(self):
        ...
        time.sleep(2)
        while len(cmds) <3:
            logger.debug("Waiting for at least 3 waypoints to be set")
            time.sleep(1)
        missionlist=[]
        ...
        logger.debug(f" ESKİ LEN OF MİSSİON ;== {len(missionlist)}")
        ...
        logger.debug(f" YENİ LEN OF MİSSİON ;== {len(missionlist)}")
        ...
        # self.mavlink_handler.simple_go_to(lat, lon, 30, block=True, distance_radius=1.5)
        # time.sleep(2)
